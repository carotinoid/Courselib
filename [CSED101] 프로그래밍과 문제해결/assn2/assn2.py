...
def computer_pattern(dice_set, score_list):
    ...
    if there_is_not(score_list[1][6]): # C 항목에 대한 전처리
        ...
        if rounds < 12 and sum(dice_set) < 20: computed_score[6] = -1  # 20점 미만일 때 C항목 무시, C만 남으면 그대로 유지.
    Cate_index = computed_score.index(max(computed_score)) # 최대 점수를 얻는 카테고리 인덱스
...
def check_error(score_list):
    ...
    if not err: # 줄 수나 입력되지 않은 값, 타입 에러가 없을시 각 점수가 가능한지 판단.
        ...
        check_num = list(tuple(i for i in range(0, 5*(j+1)+1, (j+1))) for j in range(0, 6)) # 1~6
        for i in range(6, 8): # C, 4K
            temp = tuple([i for i in range(5, 31)] + [0])
            check_num.append(temp)
        temp = tuple([i for i in range(7, 29)] + [0, 5, 30])
        check_num.append(temp) # FH
        check_num.append((0, 15)) # SS
        check_num.append((0, 30)) # LS
        check_num.append((0, 50)) # Yacht
        for i in range(0, 12):
