...
unsigned float_i2f(int x) {
   ...
   int mantissa, shift_amount, right_shift_bits, rounding_bit_pos, rounding_mask, ev_25, ev_24, ev_23, rounding, E;
   unsigned result;
   ...
   shift_amount = 23 - E;
   if(E <= 23) {
      // part 4-1. if original (int) x can be represented in 23-bit without rounding.
      // Just marking.
...
int float_f2i(unsigned uf) {
   ...
   // part 2. handling exception. Too small (including denormalized) and too large including not number
   ...
   // adding (1 << exp), because Mantissa is 1.xxx, but 1 is not represented in bits.
   result += 1 << exp; // (2 ^ exp)
