...
def find_route_dfs(dict_graph, start_station_id, end_station_id, max_depth=1000):
    """
    ...
    # BEGIN_YOUR_ANSWER
    def dfs(current_station_id, dep, time):
        if current_station_id == end_station_id:
            return
        if dep > max_depth:
            return
        for route in dict_graph[current_station_id].routes:
            next_station_id = route.station_to.id
            new_time = time + route.travel_time
            if next_station_id not in travel_time or new_time < travel_time[next_station_id]:
                travel_time[next_station_id] = new_time
                parent[next_station_id] = current_station_id
                dfs(next_station_id, dep + 1, new_time)

    dfs(start_station_id, 0, 0)
...
def find_route_iterative_deepening(dict_graph, start_station_id, end_station_id, max_depth=1000):
    ...
    for dep in range(1, max_depth):
        route, total_time = find_route_dfs(dict_graph, start_station_id, end_station_id, dep)
        if route != None:
            return route, total_time
    # END_YOUR_ANSWER
...
def find_route_uniform_cost(dict_graph, start_station_id, end_station_id):
    ...
    while True:
        ...
        for station in travel_time:
            if station not in visited and travel_time[station] < min_time:
                current_station_id = station
                min_time = travel_time[station]
        if current_station_id == None:
            break
        if current_station_id == end_station_id:
            break
        ...
        for route in dict_graph[current_station_id].routes:
            next_station_id = route.station_to.id
            # distance : 1 + d/v min, d : haversine distance, v : 60km/h
            new_time = travel_time[current_station_id] + route.travel_time
            if next_station_id not in travel_time or new_time < travel_time[next_station_id]:
                travel_time[next_station_id] = new_time
                parent[next_station_id] = current_station_id
...
def find_route_greedy(dict_graph, start_station_id, end_station_id, heuristic_type='simple'):
    ...
    while len(queue):
        current_station_id = None
        min_time = float('inf')
        queue.sort(key=lambda x: calc_heuristic(dict_graph[x], dict_graph[end_station_id], heuristic_type))
        current_station_id = queue.pop(0)
        for route in dict_graph[current_station_id].routes:
            next_station_id = route.station_to.id
            new_time = travel_time[current_station_id] + route.travel_time
...
def find_route_a_star(dict_graph, start_station_id, end_station_id, heuristic_type='simple'):
    ...
    while len(queue):
        current_station_id = None
        min_time = float('inf')
        queue.sort(key=lambda x: calc_heuristic(dict_graph[x], dict_graph[end_station_id], heuristic_type) + travel_time[x])
        current_station_id = queue.pop(0)
        for route in dict_graph[current_station_id].routes:
            next_station_id = route.station_to.id
            new_time = travel_time[current_station_id] + route.travel_time
