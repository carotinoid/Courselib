...
def get_test_1b(multiplier):
    ...
    def solve(mdp, ValueIteration, *args):
        startState = mdp.startState()
        alg = ValueIteration()
        alg.solve(mdp, *args)
        return alg.V[startState], alg

    def test_1b():
        tm = graderUtil.TimeMeasure()

        for m in range(1000, 1001):
            mdp = BlackjackMDP(cardValues=[1, 3, 5, 8, 2], multiplicity=m,
                                        threshold=50, peekCost=1)
            tm.check()
            util_vi, alg_vi = solve(mdp, util.ValueIteration, .00001)
            util_vi_time = tm.elapsed()

            tm.check()
            pred_dp, alg_dp = solve(mdp, submission.ValueIterationDP)
            pred_dp_time = tm.elapsed()
            print("Result of Test with multiplicity %d"%m)
            print('VI time: {} / DP time: {}'.format(util_vi_time, pred_dp_time))
            print("util_vi : ", util_vi)
            print("pred_dp : ", pred_dp)

            for state in mdp.states:
                v_dp = alg_dp.V[state]
                v_vi = alg_vi.V[state]
                grader.requireIsTrue((abs(v_dp-v_vi)) < 0.00001)
            
            grader.requireIsTrue((abs(pred_dp - util_vi) / util_vi) < 0.00001)
            grader.requireIsTrue(pred_dp_time * multiplier < util_vi_time)

            print("Test with multiplicity %d Succeed!\n"%m)
